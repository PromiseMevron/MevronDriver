...
@HiltViewModel
class VehicleImageUploadViewModel @Inject constructor (private val repository: MevronRepo)  : ViewModel() {

    fun uploadImage(data: MultipartBody.Part, id: String): LiveData<GenericStatus<GeneralResponse>> {
        val result = MutableLiveData<GenericStatus<GeneralResponse>>()
        CoroutineScope(Dispatchers.IO).launch {
            try{
                val response = repository.uploadVehicleImage(data, id)
