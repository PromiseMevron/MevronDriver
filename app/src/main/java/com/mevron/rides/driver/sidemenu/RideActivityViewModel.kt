...
@HiltViewModel
class RideActivityViewModel @Inject constructor (private val repository: MevronRepo) : ViewModel() {


    fun getRides(startDate: String, endDate: String): LiveData<GenericStatus<GeneralResponse>> {

        val result = MutableLiveData<GenericStatus<GeneralResponse>>()

        CoroutineScope(Dispatchers.IO).launch {
            try{
                val response = repository.getAllTrips(startDate = startDate, endDate = endDate)
