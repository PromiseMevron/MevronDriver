...
@HiltViewModel
class RideRequestViewModel @Inject constructor (private val repository: MevronRepo) : ViewModel() {

    fun tripManagement(data: TripManagementModel): LiveData<GenericStatus<GeneralResponse>> {

        val result = MutableLiveData<GenericStatus<GeneralResponse>>()

        CoroutineScope(Dispatchers.IO).launch {
            try{
                val response = repository.tripManagement(data)
