...
@HiltViewModel
class InsuranceViewModel @Inject constructor (private val repository: MevronRepo)  : ViewModel() {


    fun uploadInsurance(data: MultipartBody.Part, id: String): LiveData<GenericStatus<GeneralResponse>> {
        val result = MutableLiveData<GenericStatus<GeneralResponse>>()
        CoroutineScope(Dispatchers.IO).launch {
            try{
                val response = repository.uploadInsurance(data, id)
