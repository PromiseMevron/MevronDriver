...
import com.mevron.rides.driver.databinding.FragmentStickerBinding
import com.mevron.rides.driver.databinding.FragmentVehicleImageUploadBinding
...
@AndroidEntryPoint
class VehicleImageUploadFragment : Fragment() {

    companion object {
        fun newInstance() = VehicleImageUploadFragment()
    }
    private lateinit var binding: FragmentVehicleImageUploadBinding
    private var image: Bitmap? = null
    private var imageUri: Uri? = null
    private var mDialog: Dialog? = null
    private var uuid = ""

    private val viewModel: VehicleImageUploadViewModel by viewModels()

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_vehicle_image_upload, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        uuid = VehicleImageUploadFragmentArgs.fromBundle(requireArguments()).id
    ...
    fun uploadData() {
        ...
        if (imageUri == null) {
            Snackbar.make(binding.root, "Select Image", Snackbar.LENGTH_LONG)
        ...
        viewModel.uploadImage(body, uuid).observe(viewLifecycleOwner, Observer {
            it.let { res ->
                when (res) {
                ...
                Constants.REQUEST_TAKE_PHOTO -> {
                    try {
                        ...
                        Utils.currentPhotoPath?.let {
                            ...
                            binding.text13.text =
                                context?.resources?.getString(R.string.submit_if_readable)
