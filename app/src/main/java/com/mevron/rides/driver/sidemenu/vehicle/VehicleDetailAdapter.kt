...
class VehicleDetailAdapter(val sel: SelectVehicleDetail, val context: Context): ListAdapter<Document, VehicleDetailAdapter.VehiHolder>(
    ...
    override fun onBindViewHolder(holder: VehiHolder, position: Int) {
        ...
        when (setUpDocView(doc.status)){
            DocStatus.Okay -> {
                ...
                holder.binding.backGround.setBackgroundColor(context.resources.getColor(R.color.document_ok, null))
            }
            DocStatus.Pending -> {
                holder.binding.docStatus.visibility = View.VISIBLE
                holder.binding.backGround.setBackgroundColor(context.resources.getColor(R.color.document_under_review, null))
                holder.binding.docStatus.text = context.getString(R.string.under_review)
            }
            DocStatus.None -> {
                holder.binding.docStatus.visibility = View.VISIBLE
                holder.binding.backGround.setBackgroundColor(context.resources.getColor(R.color.document_pending, null))
                holder.binding.docStatus.text = "${context.getString(R.string.upload_photo)} ${doc.name}"
            }
        }
